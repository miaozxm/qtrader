...
- [x] **V0.2 选股器**：全市场策略扫描（技术指标多条件筛选），一键生成候选池
- [x] **V0.3 组合监控**：自选股管理、多标的组合净值与回撤、策略信号面板
- [x] **V0.4 策略实验室**：参数寻优（grid search）、多周期多市场扩展
- [x] **V0.5 模拟交易**：纸面交易账户（现金/持仓/成交），练习策略不涉及真实资金
- [ ] **V0.6 实盘对接**：对接 vn.py 生态（vnpy_ctp / vnpy_xtp）交易接口
...
- 📊 **行情分析**：A股/港股 K线、实时行情、MA/MACD/RSI/KDJ/BOLL、单标的回测
- 🎯 **选股器**：全市场扫描，支持多条件组合（均线多头 / MACD金叉 / RSI超卖 / 布林突破 / 放量上涨），结果可导出 CSV
- 💼 **组合监控**：自选股管理、多标的等权组合净值与回撤曲线、多策略实时信号面板
- 🧪 **策略实验室**：网格参数寻优（双均线/MACD/RSI），按收益/夏普/回撤排序，导出 CSV
- 🖥 **模拟交易**：虚拟资金纸面交易，持仓/成交/盈亏记录，策略练手不担风险
